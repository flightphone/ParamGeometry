...
import {
  ...
  mod, vec3s, vec3m, rotz, max, min, mix, smin, smin_out, PI, TAU
} from "../CommonGLSL";
...
function cylinder2(p)
{
  let t = cylinder(p, 0.7, 2.);
  let t2 = cylinder(vec3(p.y, p.z, p.x), 0.7, 2.);
  t = smin(t, t2, 0.1);
  t2 = cylinder(vec3(p.x, p.z, p.y), 0.7, 2.);
  t = smin(t, t2, 0.1);
  
  
  /*
  t = max(t, -cylinder(p, 0.3, 1.8)) ;
  t = max(t, -cylinder(vec3(p.y, p.z, p.x), 0.3, 1.8));
  t = max(t, -cylinder(vec3(p.x, p.z, p.y), 0.3, 1.8));
  */
  /*
  t = min(-t, cylinder(p, 0.3, 3.)) ;
  t = min(-t, cylinder(vec3(p.y, p.z, p.x), 0.3, 3.));
  t = min(-t, cylinder(vec3(p.x, p.z, p.y), 0.3, 3.));
  */
  t = smin_out(cylinder(p, 0.3, 2.5), t, 0.15) ;
  t = smin_out(cylinder(vec3(p.z, p.y, p.x),0.3, 2.5), t, 0.15) ;
  t = smin_out(cylinder(vec3(p.x, p.z, p.y),0.3, 2.5), t, 0.15) ;
  return t-0.1;
}
export { map, cylinder2 }